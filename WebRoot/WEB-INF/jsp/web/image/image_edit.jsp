				...
				<div class="row">
					...
					<form action="image/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								<td style="width:79px;text-align: right;padding-top: 13px;">案例分类:</td>
								<td id="juese">
									<select class="chosen-select form-control" name="PRODUCT_TYPE" id="PRODUCT_TYPE" data-placeholder="请选择案例分类" style="vertical-align:top;" style="width:98%;" >
										<option value=""></option>
										<c:forEach items="${CPFL}" var="FL">
											<option value="${FL.DICTIONARIES_ID }" <c:if test="${FL.DICTIONARIES_ID == pd.PRODUCT_TYPE }">selected</c:if>>${FL.NAME }</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">背景颜色:</td>
								<td><input type="text" name="BG_COLOR" id="BG_COLOR" value="${pd.BG_COLOR}" maxlength="100"  title="背景颜色"  /></td>
							</tr>
							<tr>
